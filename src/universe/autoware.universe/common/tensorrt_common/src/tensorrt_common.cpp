...
bool TrtCommon::setTensorAddress(const char * tensor_name, void * data)
{
  ...
  if (!success) {
    logger_.log(nvinfer1::ILogger::Severity::kERROR, "Failed to set tensor address for tensor");   
  }
  return success;
}
  ...
  const std::string & onnx_file_path, const std::string & output_engine_file_path)
{
  ...
  if (build_config_->dla_core_id != -1) {
    if (num_available_dla > 0) {
      ...
      //std::cout << "###Warning : "
      //          << "No DLA is supported! ###" << std::endl; //tues add
  ...
  if (batch_config_.at(0) > 1 && (batch_config_.at(0) == batch_config_.at(2))) {
    ...
    //builder->setMaxBatchSize(batch_config_.at(2)); //wheeltec:tues NV_TENSORRT_MAJOR < 8
...
//wheeltec tues modified the getBindingDimensions function :
nvinfer1::Dims TrtCommon::getBindingDimensions(const int32_t index) const
{
 ...
 if (has_runtime_dim) {
    //保留逻辑判断,记录日志，或用于调试目的
    std::cerr << "Warning: Runtime dimension detected for binding index " << index << "\n";
  }
  return dims;
#else
...
//wheeltec tues add (start)
#if (NV_TENSORRT_MAJOR * 1000 + NV_TENSORRT_MINOR * 100 + NV_TENSOR_PATCH * 10) >= 10300
int32_t TrtCommon::getNbIOTensors()
{
...
#endif
//wheeltec tues add (end)

bool TrtCommon::setInputShape(const int32_t index, const nvinfer1::Dims & dimensions) const
{
  //return context_->setInputShape(index, dimensions);
  //wheeltec tues changes
    const char* tensorName = engine_->getIOTensorName(index);
    if (!tensorName) {
        throw std::runtime_error("Invalid binding index.");
    }
    return context_->setInputShape(tensorName, dimensions); 
}
/*
...
bool TrtCommon::setInputShape(const char * tensor_name, const nvinfer1::Dims & dimensions) 
{
  ...
  if (!success) {
  logger_.log(nvinfer1::ILogger::Severity::kERROR, "Failed to set input shape for tensor");
  }
  return success;
}
*/
