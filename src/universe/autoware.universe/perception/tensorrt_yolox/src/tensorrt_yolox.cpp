  ...
  const size_t max_workspace_size)
{
  ...
  //switch (trt_common_->getNbBindings()) { //wheeltec tues changes
...
void TrtYoloX::initPreprocessBuffer(int width, int height)
{
  ...
  src_height_ = height;
  if (use_gpu_preprocess_) {
    auto input_dims = trt_common_->getTensorShape(0);//wheeltec tues changes
    ...
    if (!image_buf_h_) {
      trt_common_->setInputShape(0, input_dims);//wheeltec tues changes
      scales_.clear();
    }
...
bool TrtYoloX::feedforward(const std::vector<cv::Mat> & images, ObjectArrays & objects)
{
  ...
  //trt_common_->enqueueV2(buffers.data(), *stream_, nullptr);//wheeltec tues changes
