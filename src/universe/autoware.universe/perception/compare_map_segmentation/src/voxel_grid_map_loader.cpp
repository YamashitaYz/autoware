...
void VoxelGridDynamicMapLoader::timer_callback()
{
  ...
  if (last_updated_position_ == std::nullopt) {
    RCLCPP_INFO(logger_, "111111111111111111111111111111111111111..\n");
  ...
  if (should_update_map()) {
    RCLCPP_INFO(logger_, "222222222222222222222222222222222222222...\n");
    last_updated_position_ = current_position_;
    request_update_map((current_position_.value()));
    last_updated_position_ = current_position_;
  }
...
void VoxelGridDynamicMapLoader::request_update_map(const geometry_msgs::msg::Point & position)
{
  ...
  auto result{map_update_client_->async_send_request(
    request,
    [](rclcpp::Client<autoware_map_msgs::srv::GetDifferentialPointCloudMap>::SharedFuture) {std::cout<<"ddddd"<<std::endl;})};

  std::future_status status = result.wait_for(std::chrono::seconds(0));
  while (status != std::future_status::ready) {
    RCLCPP_INFO(logger_, "Waiting for response...\n");
    if(status == std::future_status::timeout)RCLCPP_INFO(logger_, "response timeout...\n");
    if(status == std::future_status::deferred)RCLCPP_INFO(logger_, "response deferred...\n");
    if (!rclcpp::ok()) {
      return;
    }
