...
void CenterPointTRT::inference()
{
  ...
  //encoder_trt_ptr_->context_->enqueueV2(encoder_buffers.data(), stream_, nullptr); //wheeltec tues
  encoder_trt_ptr_->setTensorsAddresses(encoder_buffers);
  ...
  head_trt_ptr_->setTensorsAddresses(head_buffers);
