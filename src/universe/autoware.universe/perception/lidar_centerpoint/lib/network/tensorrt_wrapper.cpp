...
//wheeltrec tues add 
bool TensorRTWrapper::setInputShape(const int32_t index, const nvinfer1::Dims & dimensions) const
{
...
bool TensorRTWrapper::setTensorAddress(const int32_t index, void * data)
{
  auto const & name = engine_->getIOTensorName(index);
  return setTensorAddress(name, data);
}

bool TensorRTWrapper::setTensorAddress(const char * tensor_name, void * data)
{
  if (!context_) {
...
bool TensorRTWrapper::setTensorsAddresses(std::vector<void *> & tensors)
{
  bool success = true;
...
bool TensorRTWrapper::setTensorsAddresses(std::unordered_map<const char *, void *> & tensors)
{
  bool success = true;
...
void TensorRTWrapper::setupBindings(std::vector<void *> & bindings)
{
